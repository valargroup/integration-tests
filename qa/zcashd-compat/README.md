...
The CI job (`test-rpc-zcashd-compat`) is required when it runs. Failures block the workflow.
...
| `zcashd_compat_reorg.py` | Reorg following: zebrad-side `invalidateblock` plus a longer replacement chain at reorg depths 3 and 25 (the larger approaching the 30-block sync batch size), asserting zcashd reorgs to the new tip and reports the old chain as an orphaned `valid-fork` with negative confirmations. |
| `zcashd_compat_reorg_wallet.py` | A confirmed wallet transaction survives a zebrad-side reorg: the confirming block is replaced by a longer competing chain, the tx returns to the mempool and is re-mined into a new block, and the wallet re-confirms the deposit (orphaned block at -1, new blockhash, no double-count). Single node. Note: the wrapper deliberately does not follow a strict truncation (zebra regressing below the trusted boundary leaves it in `sync state = degraded` holding the confirmed chain), so a durable de-confirm only arises from an actual double-spend, which is out of scope here. |
| `zcashd_compat_persistence.py` | Ingested chainstate survives an unclean restart: with `-zebra-compat-flush-interval=1` the compat ingest path flushes the coins DB to disk (`chainstate_flush.last_flushed_height` tracks the tip — the fix for the restart-replay bug); zcashd is then `SIGKILL`ed and restarted against the still-running paired zebrad, and must come back tip-aligned with the persisted chainstate. |
